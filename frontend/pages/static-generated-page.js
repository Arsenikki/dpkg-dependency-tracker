      ...
      <div class='flex flex-row py-6 px-6 pt-20 text-md bg-gray-800'>
        <div class='p-6 w-4/12 min-w-sm max-w-md'>
          {packages.map((pkg, index) => (
            <div
              key={index}
              class='mb-4 bg-gray-100 rounded shadow-md text-center text-lg'
            >
              <button
        ...
        {selectedPkg ? (
          <div class='p-6 w-full'>
            <div class='w-full top-6 p-8 sticky bg-gray-100 rounded shadow-md border border-gray-400 z-0'>
              ...
              <p class='py-2 whitespace-pre-line'>{selectedPkg.Description}</p>
...
export async function getStaticProps() {
  ...
  const res = await fetch(`https://untappiappi.westeurope.cloudapp.azure.com/dpkg/api/api/packages`); //http://dpkg-backend-clusterip/api/packages:5000
  const packages = await res.json();
  console.log("jälkeen", packages)
  return {
    props: {
      packages,
    },
